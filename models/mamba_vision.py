...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        dpr_decoder = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))][::-1] # encoder 和 decoder dpr 相反
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            up_layer = MambaVisionLayer_up(dim=int( dim * 2 ** (3-i) ),
                                     ...
                                     layer_scale=layer_scale,
                                     layer_scale_conv=layer_scale_conv,
                                     transformer_blocks=list(range(depths[3-i]//2+1, depths[3-i])) if depths[3-i]%2!=0 else list(range(depths[3-i]//2, depths[3-i])), 
                                     )
